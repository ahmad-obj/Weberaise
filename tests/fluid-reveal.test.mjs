...
test('fluid exit source is deterministic fullscreen injection without periodic wave math', async () => {
  ...
  assert.match(shaders, /shapePhase\s*=\s*smoothstep\(0\.12,\s*0\.88,\s*progress\)/);
  assert.match(shaders, /velocityDrive\s*=\s*mix\(0\.18,\s*0\.26,\s*drive\)/);
  assert.match(shaders, /velocityDriven\s*=\s*mix\(base,\s*velocityTarget,\s*velocityDrive\)/);
  ...
  assert.equal(exit.EXIT_FLUID_CONFIG.velocityPeak, 8.0);
  assert.equal(exit.EXIT_FLUID_CONFIG.lateralStrength, 0.45);
  assert.equal(exit.EXIT_FLUID_CONFIG.sealStart, 0.9997);
