...
- [x] **Step 3: Commit the approved design documents and failing contracts only**
...
- [x] **Step 1: Render one decorative SVG per service row**
...
- [x] **Step 2: Replace color interpolation with transform-only travel**
...
- [x] **Step 3: Make title/number contrast follow the wave edge**
...
- [x] **Step 1: Add one centralized surface lead constant**

Add `surfaceLead: 0.16` to `SERVICES_MOTION.hover` and assert it in the focused test.
...
- [x] **Step 4: Ensure click establishes the final surface**
...
- [x] **Step 1: Capture precise desktop wave frames**
...
- [x] **Step 2: Verify every title length**
...
- [x] **Step 3: Verify combined pointer and focus**
...
- [x] **Step 4: Verify selected-row continuity**
...
- [x] **Step 1: Verify tablet and mobile**
...
Final evidence: the focused Services suite, typecheck, production build, and `git diff --check` pass. The complete suite remains 15/18 with the same baseline-only failures in `experience.test.mjs`, `interaction-polish.test.mjs`, and `visual-contract.test.mjs`; `services-page.test.mjs` passes within the full run.

- [x] **Step 5: Mark this plan complete and commit the refinement**
